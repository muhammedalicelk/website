...
// Dosya Trimmer Component (FIX'li)
function DosyaTrimmer({ dosya, onRemove, onUpdate }) {
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      ...
      // ✅ trimEnd/trimStart'ı her event'te resetleme
      // sadece ilk kurulumda ayarla
    ...
    // ✅ önemli: duration/isReady değişince effect yeniden çalışmasın (reset yapmasın)
  }, [dosya.id, onUpdate]);
  ...
  // ✅ clamp: start/end birbirini geçmesin, slider "takılmasın"
  const handleStartChange = (e) => {
    const newStart = parseFloat(e.target.value);
    const clamped = Math.min(newStart, dosya.trimEnd - 0.1);
    onUpdate(dosya.id, { trimStart: clamped });
  };

  const handleEndChange = (e) => {
    const newEnd = parseFloat(e.target.value);
    const clamped = Math.max(newEnd, dosya.trimStart + 0.1);
          ...
          <div className="flex-1 min-w-0">
            ...
            {!dosya.isReady ? (
              ...
              <span className="text-xs text-green-600">
                ✓ Hazır - Toplam: {formatTime(dosya.duration)} - Şimdi oynatabilirsiniz!
              </span>
            )}
          </div>
        </div>
